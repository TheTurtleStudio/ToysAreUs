...
class GoToMainMenu(): #Change this to the name of your script
    ...
    def Update(self):
        ...
        if (self.engine.Input.TestFor.RIGHTMOUSEDOWN()):
            if (self.gameObject.sprite.rect in self.engine.Collisions.PointCollide(self.engine.Input.TestFor.MOUSEPOS(), [Types.CollisionLayer.UI])):
                self.engine.SetUniversal("STARTED", False)
                self.engine.Reload()
...
class Create():
    def __init__(self, engine):
        self.obj: GoToMainMenu = GoToMainMenu(engine) #Replace Template with the name of your class
